...
class SimpleExecutor(Executor):
    ...
    def Transform(self, inputs: Mapping[Text, Any],
        ...
        This is the implementation part of transform executor. This is
        intended for
        ...
        Args:
          inputs: A dictionary of labelled input values, including:
            ...
            - labels.ANALYZE_DATA_PATHS_LABEL: Paths or path patterns to
            analyze
              data.
            - labels.ANALYZE_PATHS_FILE_FORMATS_LABEL: File formats of paths to
              analyze data.
            - labels.TRANSFORM_DATA_PATHS_LABEL: Paths or path patterns to
            transform
              data.
            - labels.TRANSFORM_PATHS_FILE_FORMATS_LABEL: File formats of
            paths to
              transform data.
            - labels.MODULE_FILE: Path to a Python module that contains the
              preprocessing_fn, optional.
            - labels.PREPROCESSING_FN: Path to a Python function that
            implements
              preprocessing_fn, optional.
        ...
        if len(transform_data_paths) != len(transform_paths_file_formats):
            raise ValueError(
                ...
                'transform_paths_file_formats do not match: {} v.s {}'.format(
                    ...
                    len(transform_paths_file_formats)))
        ...
        # Inspecting the preprocessing_fn even if we know we need a full
        # pass in
        ...
        if (not compute_statistics and not materialize_output_paths and
            ...
            if analyze_input_columns:
                absl.logging.warning(
                    'Not using the in-place Transform because the following '
                    'features '
                    'require analyzing: {}'.format(
                        tuple(c for c in analyze_input_columns)))
            else:
                absl.logging.warning(
                    'Using the in-place Transform since '
                    'compute_statistics=False, '
                    'it does not materialize transformed data, and the '
                    'configured '
                    'preprocessing_fn appears to not require analyzing the '
                    'data.')
        ...
        stats_options_updater_fn = (stats_options_updater_fn
                                    if stats_options_updater_fn else lambda _,
                                                                            x: x)

        materialization_format = (
            transform_paths_file_formats[
                -1] if materialize_output_paths else None)
